...
export async function drawHslWheel(
	...
	console.log(colors);
	const canvas = createCanvas(size, size + palletteSize);
	...
	for (const col of colors) {
		...
		ctx.beginPath();
		ctx.arc(mx, my, markerRadius, 0, Math.PI * 2, true);
	...
	for (let i = 0; i < colors.length; i++) {
		...
		console.log(h, s, l);
		ctx.fillStyle = `hsl(${h}, ${s * 100}%, ${l * 100}%)`;
		ctx.fillRect(i * rectWidth, stripY, rectWidth, palletteSize);
		ctx.lineWidth = 2;
		ctx.strokeStyle = 'black';
		ctx.strokeRect(i * rectWidth, stripY, rectWidth, palletteSize);
	}
